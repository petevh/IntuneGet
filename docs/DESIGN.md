...
> **Doc location (2026-07-25):** this note and the catalog scripts moved from the
> `docker-homelab` repo into the IntuneGet source repo (`docs/`, `scripts/`) — we
> now own the fork's engineering, so the design lives with the code. The homelab
> repo keeps only compose/deploy glue + the `refresh-catalog.sh` cron wrapper.

| State | Item |
|---|---|
| **Done** | Web app live — catalog browsing, TLS via Traefik, MSAL sign-in verified, no Supabase, secret local. On `main`, pushed. |
| **Done** | Packager payload fixed — five Graph-payload bugs, now on `main` (`4d0792f93`, was fork branch `fix/packager-win32lobapp-create-payload`). Deploys succeed against a live tenant. |
| **Done (2026-07-25)** | Native-first detection is ProductCode-driven — winget apps detect on the uninstall-registry key (real winget `ProductCode`, root-level merged onto each installer) or a folder rule; custom apps keep the IntuneGet marker (`bb2f16d3d`). `getBestInstaller` prefers a ProductCode-bearing installer node. Web suite 510 green. Fixed the bogus `%ProgramFiles%\<DisplayName>` folder guess (e.g. Adobe Acrobat). See UPSTREAM-ISSUES.md #3. |
| **Done (2026-07-25)** | Packager ↔ web detection contract harmonized — `feat/packager-intunewin32app` merged to `main` (native `IntuneWin32App` build path; PSADT kept as tier-5 fallback). Native-build gating (`usesNativeBuild`/`isMarkerDetectionRule`) verified against the ProductCode rewrite: it is keyPath-exact-match, not type-based, so winget→native / custom→marker→PSADT routes correctly (`dfdcdbae2`, with regression tests). Silent-args fallback (`packager/src/silent-args.ts`, §3) landed with it. |
...
| **Partly done** | Packaging redesign — the `IntuneWin32App` native build path is merged (§2) and silent-args fallback (§3) landed. **Still open:** where the build runs (GitHub runner vs. wake-on-demand VM, see below) and the upload-half decision (IntuneWin32App upload vs. the fork's own upload code). |
