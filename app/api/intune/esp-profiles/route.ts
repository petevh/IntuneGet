...
export async function GET(request: NextRequest) {
  try {
    ...
    // MSP tenant resolution and the tenant_consent gate are Supabase-backed. On a
    // self-hosted instance without Supabase there is no MSP org and no consent
    // table, so use the token's own tenant and rely on the sign-in Graph consent
    // check. createServerClient() throws when Supabase is unconfigured, so only
    // touch it when configured (see UPSTREAM-ISSUES.md #5).
    let tenantId: string;
